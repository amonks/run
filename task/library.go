...
// collection of [Task]s. You can create a [runner.Run] by passing a Library into
// [runner.New].
type Library struct {
	ids      []string
...
// ValidateWithCWD inspects a Library and returns an error if it is invalid,
// using the given working directory for path validation.
func (lib Library) ValidateWithCWD(cwd string) error {
	v, err := newValidatorWithCWD(cwd)
	if err != nil {
		return err
	}
	return v.validate(lib)
}
