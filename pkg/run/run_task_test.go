...
func TestShortRunNoDepsSucceeds(t *testing.T) {
	...
	task := fixtures.NewTask("build", "short")

	cancel, errs := startRun(t, []run.Task{task}, "build", mw)
	defer cancel()
...
func TestShortRunDependencyChain(t *testing.T) {
	...
	cancel, errs := startRun(t, []run.Task{t1, t2, t3}, "link", mw)
	defer cancel()
...
func TestFailingDependencyPreventsDependents(t *testing.T) {
	...
	cancel, errs := startRun(t, []run.Task{t1, t2}, "build", mw)
	defer cancel()
...
func TestContextCancellation(t *testing.T) {
	mw := fixtures.NewWriter()
	task := fixtures.NewTask("server", "long").WithCancel(context.Canceled)

	cancel, errs := startRun(t, []run.Task{task}, "server", mw)
...
func TestWatchTriggeredRestartLongTask(t *testing.T) {
	...
	task := run.FuncTask(func(ctx context.Context, onReady chan<- struct{}, w io.Writer) error {
		startCount.Add(1)
	...
	mw := fixtures.NewWriter()
	_, cancel, errs := startRunWithHandle(t, []run.Task{task}, "server", mw)
	defer cancel()
...
func TestTriggerCompletionCausesRestart(t *testing.T) {
	...
	var startCount atomic.Int32
	main := run.FuncTask(func(ctx context.Context, onReady chan<- struct{}, w io.Writer) error {
		startCount.Add(1)
		w.Write([]byte("! server: start\n"))
		close(onReady)
		<-ctx.Done()
	...
	_, cancel, errs := startRunWithHandle(t, []run.Task{trigger, main}, "server", mw)
...
func TestWatchEventOnTriggerCausesCascade(t *testing.T) {
	...
	_, cancel, errs := startRunWithHandle(t, []run.Task{trigger, main}, "app", mw)
	defer cancel()
...
func TestDepRerunDoesNotRestartLongTask(t *testing.T) {
	...
	_, cancel, errs := startRunWithHandle(t, []run.Task{dep, main}, "server", mw)
	defer cancel()
...
func TestJSONOutputPrettified(t *testing.T) {
	mw := fixtures.NewWriter()
	task := fixtures.NewTask("json-task", "short").
		WithOutput("{\"key\":\"value\",\"nested\":{\"a\":1}}\n")

	cancel, errs := startRun(t, []run.Task{task}, "json-task", mw)
	defer cancel()
...
func TestLongTaskOnReadyEnablesDependent(t *testing.T) {
	...
	_, cancel, errs := startRunWithHandle(t, []run.Task{dep, main}, "api", mw)
	defer cancel()
...
func TestDynamicAdd(t *testing.T) {
	...
	r, cancel, errs := startRunWithHandle(t, []run.Task{main, extra}, "server", mw)
	defer cancel()
...
func TestDynamicRemove(t *testing.T) {
	...
	r, cancel, errs := startRunWithHandle(t, []run.Task{dep, main}, "server", mw)
	defer cancel()
...
func TestInvalidateRestartsRunningTask(t *testing.T) {
	...
	r, cancel, errs := startRunWithHandle(t, []run.Task{task}, "worker", mw)
	defer cancel()
