...
type Runner struct {
	...
	waiters        map[string]*waiter.Waiter
...
func (r *Runner) Run(ctx context.Context, requestedTaskIDs ...string) error {
	...
	// Clean up.
	defer r.mu.Lock("cleanup").Unlock()
...
func (r *Runner) deactivateSubtree(id string) error {
	...
	// Cancel tasks that should be stopped.
	{
		...
		for _, id := range toCancel {
			if waiter, hasWaiter := r.waiters[id]; hasWaiter {
...
func (r *Runner) runTask(ctx context.Context, id string) error {
	...
	wtr, hasWaiter := r.waiters[id]
	if hasWaiter && !wtr.IsDone() {
		r.mu.Printf("canceling %s before re-running it", id)
		go func() { wtr.Cancel(); r.input <- msgRunTask(id) }()
		return nil
	}
	...
	// Create a waiter for the task.
	onReady := make(chan struct{})
	wtr = waiter.New(func(ctx context.Context) error {
	...
	go func() {
		...
		wtr.Start()
		err, notCanceled := <-wtr.Wait()

		// Done.

		// If this task has already been replaced, do nothing.
		r.mu.Lock("destroy waiter")
		if !r.waiters[id].Is(wtr) {
			r.mu.Unlock()
			return
		}
		// Destroy the waiter.
		...
		// If we were canceled, notify the canceler. Otherwise, back to
		// the main loop.
		if notCanceled {
